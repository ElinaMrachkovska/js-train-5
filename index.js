...
function printIndexes(array) {
  ...
  // function printIndexes(array) {
  //      const printIndexes = array;
  //   for (array in printIndexes) {
...
function modifyArray(array) {
  ...
  const modifiedArray = array.toString();
...
function printStrings(array) {
  ...
  const stringifiedArray = array.toString().replace(/ /g, "");

  // Повторюємо рядок 2 рази
  const result = stringifiedArray.repeat(2);
...
function concatenateStrings(array, subString) {
  ...
  const stringifiedArray = array.toString();

  // Перевіряємо, чи містить рядок підрядок
  const isSubStringPresent = stringifiedArray.includes(subString);
...
function addTenToEach(array) {
  ...
  for (const element in array) {
    // Додаємо до поточного елемента 10
    const newElement = element + 10;

    // Додаємо результат в новий масив
